package com.blackbeard.admin.controller.system.login;
...
public class LoginController extends BaseController {
	...
	public void updateUserState(String USERNAME, String userState)
		...
		userService.updateUserState(pd);
	}
		...
		if (null != KEYDATA && KEYDATA.length == 3) {
			...
			if (null == code || "".equals(code)) {
				...
				if (Tools.notEmpty(sessionCode)
					...
					String passwd = MD5Util.base64Encode(PASSWORD);
					pd.put("PASSWORD", passwd);
					pd = userService.getUserByNameAndPwd(pd);
					if (pd != null) {
						pd.put("LAST_LOGIN", DateUtil.getTime().toString());
						//userService.updateLastLogin(pd); 登录不更新这个last_login 字段，PC端登录用户才更新
						String userState = pd.getString("user_state");
						if (userState.equals("0")
								&& !pd.getString("USERNAME").equals("admin")) {// 未审核
							errInfo = "stateerror";
							map.put("result", errInfo);
							return AppUtil.returnObject(new PageData(), map);
						}
						User user = new User();
						user.setUserState(Integer.valueOf(pd
								.getString("user_state")));
						...
						user.setMt4Id(pd.getString("mt4_id"));
	...
	public ModelAndView login_index(
			@PathVariable("changeMenu") String changeMenu) throws IOException {
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
