...
<script type="text/javascript">
	...
	function save() {
	    ...
	    $.ajax({
            cache: true,
            type: "POST",
            url:'<%=basePath%>currencyConfigController/edit.do',
            data:$('#currencyConfigEditForm').serialize(),// 你的formid
            async: false,
	        ...
	        if (data.success == true) {
					alert(data.msg);
					location.href='<%=basePath%>currencyConfigController/edit.do';    
				} else {
					result=false;
					alert(data.msg);
				    location.href='<%=basePath%>currencyConfigController/editPage.do?id='+data.obj.id; 
				}
			}
		});
		$("#zhongxin").hide();
...
<body>
	<form name="currencyConfigEditForm" id="currencyConfigEditForm" method="post">
			...
			<table id="table_report" class="table table-bordered table-hover">
				<tr>
					...
					<td><input type="text" name="currency" id="currency"
						placeholder="这里输入货币名称" title="名称" style="width: 158px;"  value="${currencyConfigDto.currency}" readonly="readonly"/></td>
				</tr>
				<tr>
					<td style="width: 90px; text-align: center; padding-top: 13px;">
					...
					<td><input type="text" name="currencyRates" id="currencyRates"
						placeholder="这里输入货币利率" title="名称" style="width: 158px;" value="${currencyConfigDto.currencyRates}" /></td>
				</tr>
			</table>
