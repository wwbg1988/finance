...
<body>
	<form action="menu/edit.do" name="menuForm" id="menuForm" method="post">
		...
		<input type="hidden" name="pId" id="pId" value="${pd.PARENT_ID}" /> <input
			type="hidden" name="MENU_TYPE" id="MENU_TYPE"
			value="${pd.MENU_TYPE }" />
		<div id="zhongxin">
			<table>
				<tr>
					<td><select name="PARENT_ID" id="parentId" onchange="setMUR()"
						title="菜单">
				...
				<tr>
					<td><input type="text" name="MENU_NAME" id="menuName"
						placeholder="这里输入菜单名称" value="${pd.MENU_NAME }" title="名称" /></td>
				</tr>
				<tr>
					<td><input type="text" name="MENU_URL" id="menuUrl"
						placeholder="这里输入链接地址" value="${pd.MENU_URL }" title="地址" /></td>
				</tr>
				<tr>
					<td><input type="number" name="MENU_ORDER" id="menuOrder"
						placeholder="这里输入序号" value="${pd.MENU_ORDER}" title="序号" /></td>
				</tr>
				<tr>
					<td style="text-align: center;"><label
						style="float: left; padding-left: 32px;"><input
							name="form-field-radio" id="form-field-radio1"
							onclick="setType('1');"
							<c:if test="${pd.MENU_TYPE == '1' }">checked="checked"</c:if>
							type="radio" value="icon-edit"><span class="lbl">系统菜单</span></label>
						<label style="float: left; padding-left: 5px;"><input
							name="form-field-radio" id="form-field-radio2"
							onclick="setType('2');"
							<c:if test="${pd.MENU_TYPE != '1' }">checked="checked"</c:if>
							type="radio" value="icon-edit"><span class="lbl">业务菜单</span></label>
					</td>
				</tr>
				<tr>
