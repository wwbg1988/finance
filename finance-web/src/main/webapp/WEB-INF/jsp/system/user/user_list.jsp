	...
	<div class="row-fluid">
			...
			<table>
				<tr>
					...
					<td style="vertical-align: top;"><select title="用户状态"
									class="chzn-select" name="user_state" id="user_state" data-placeholder="请选择用户状态"
									style="vertical-align: top; width: 120px;">
										<c:forEach items="${stateMap}" var="state">
											<option value="${state.key}"
												<c:if test="${pd.user_state==state.key}">selected</c:if>>${state.value}</option>
										</c:forEach>
					...
					<c:when test="${not empty userList}">
						...
						<c:forEach items="${userList}" var="user" varStatus="vs">
							...
							<%-- 	<td>${user.NUMBER }</td> --%>
								...
								<td>${user.NAME }</td>
								<td>${user.PHONE }</td>
						        <td class="center"><span class="btn btn-small btn-danger "  style="width: 71.5px;"><fmt:formatNumber value=" ${user.blance/100}" pattern="#.##" minFractionDigits="2" > </fmt:formatNumber></span></td>
								<td>${user.ROLE_NAME }</td>
								<td>${user.certificate_no }</td>
								<td  class="center"><c:if test="${not empty user.certificate_url}">
										<img src="<%=baseIp%>${user.certificate_url}"
											name="certificateUrl" style="width: 90px; height: 60px;">
									</c:if>
								</td>
								...
								<c:if test="${user.user_state == 0 }">
								 <span class="btn btn-small btn-warning " style="width: 52px;">  待审核</span>
								</c:if> <c:if test="${user.user_state == 1 }">
								  	 <span class="btn btn-small btn-info "> 注册完成</span>
								</c:if>
							    <c:if test="${user.user_state == 2 }">
								  	 <span class="btn btn-small btn-purple " style="width: 52px;"> 在线</span>
								</c:if>
								  <c:if test="${user.user_state == 3 }">
								  	 <span class="btn btn-small btn-success " style="width: 52px;"> 离线</span>
								</c:if>
								</td>
							    ...
							    <td style="width: 58px;">
									...
									<!-- 	审核通过的不显示该按钮 -->
									<c:if test="${user.user_state == 0 }">
									<a class='btn btn-mini btn-danger' title="审核" onclick="auditUser('${user.USER_ID}')">审核</a>
									</c:if>
								    <a class='btn btn-mini btn-purple' title="充值" onclick="rechargeMoney('${user.USER_ID}','${user.user_state}')">充值</a>
...
<!-- 					<a title="批量删除" class="btn btn-small btn-danger" onclick="makeAll('确定要删除选中的数据吗?');" ><i class='icon-trash'></i></a>
 -->					</c:if>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function rechargeMoney(id,state){
			if(state==0){
				  alert("该用户未通过审核，无法充值。");
				  return false;
			}
			 ...
			 diag.Drag=true;
			 diag.Title ="充值金额页面";
			 diag.URL = '<%=basePath%>web/user/goRechargeMoney.do?USER_ID='+id;
			 diag.Width = 365;
			 diag.Height = 145;
